                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
                    ...
                    <div class="relative sm:col-span-2 lg:col-span-1 xl:col-span-1">
                        ...
                        <datalist id="trafo_list">
                            @foreach($trafos as $trafo) <option data-id="{{ $trafo->id }}" value="{{ $trafo->id_gardu }}">
                            {{ Str::limit($trafo->alamat, 30) }}</option> @endforeach
                        </datalist>
                    </div>
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-white/5 transition group">
                                ...
                                <div class="flex flex-col gap-2">
                                    ...
                                    <div>
                                        @if($pju->status === 'meterisasi')
                                            <span
                                                class="inline-flex items-center px-2.5 py-0.5 rounded text-xs font-medium bg-blue-100 text-blue-800">Meterisasi</span>
                                        @elseif($pju->status === 'non_meterisasi')
                                            <span
                                                class="inline-flex items-center px-2.5 py-0.5 rounded text-xs font-medium bg-orange-100 text-orange-800">Non-Meter</span>
                                        @else
                                            <span
                                                class="inline-flex items-center px-2.5 py-0.5 rounded text-xs font-medium bg-red-100 text-red-800">Ilegal</span>
                                        @endif
                                    </div>
                                    <div>
                                        @if($pju->verification_status == 'verified')
                                            <span class="flex items-center gap-1 text-xs font-bold text-green-600"><svg
                                                    class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                        d="M5 13l4 4L19 7"></path>
                                                </svg> Verified</span>
                                        @elseif($pju->verification_status == 'rejected')
                                            <span class="flex items-center gap-1 text-xs font-bold text-red-600"><svg
                                                    class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                        d="M6 18L18 6M6 6l12 12"></path>
                                                </svg> Rejected</span>
                                        @else
                                            <span class="flex items-center gap-1 text-xs font-bold text-yellow-600"><svg
                                                    class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <td class="px-6 py-4 align-top">
                                <div class="flex flex-col gap-1">
                                    <span class="text-sm font-medium text-gray-900 dark:text-white line-clamp-3"
                                        title="{{ $pju->alamat }}">
                                        {{ $pju->alamat }}
                                    </span>
                            ...
                            <td class="px-6 py-4 align-top">
                                <div class="space-y-2">
                                    ...
                                    <div>
                                        <span
                                            class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium {{ $pju->kondisi_lampu == 'baik' ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700' }}">
...
@push('scripts')
    <script>
        // Reuse Script AJAX & Datalist (Sama persis dengan meterisasi.blade.php)
        ...
        function updateTrafoId(inputElement) {
            ...
            if (!val) { hiddenInput.value = ''; form.submit(); return; }
