<div class="relative" x-data="{
    ...
    <button class="flex items-center text-gray-700 dark:text-gray-400" @click.prevent="toggleDropdown()" type="button">
        ...
        <span class="block mr-1 font-medium text-theme-sm">{{ Str::limit(auth()->user()->name, 10) }}</span>
    ...
    <div x-show="dropdownOpen" x-transition:enter="transition ease-out duration-100"
        ...
        <div>
            ...
            <span
                class="mt-0.5 block text-theme-xs text-gray-500 dark:text-gray-400">{{ auth()->user()->email ?? '@' . auth()->user()->username }}</span>
            <span
                class="mt-1 inline-flex items-center rounded-md bg-brand-50 px-2 py-1 text-xs font-medium text-brand-700 ring-1 ring-inset ring-brand-700/10 dark:bg-brand-400/10 dark:text-brand-400 dark:ring-brand-400/20">
        ...
        <form method="POST" action="{{ route('logout') }}" x-ref="logoutForm">
            @csrf
            <a href="#" @click.prevent="$refs.logoutForm.submit()"
